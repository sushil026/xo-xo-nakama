  ...
  return (
    ...
    >
      <div style={{ opacity: 0.15 }}>
        <PodiumSvg color="var(--amber)" size={68} />
      </div>
      <div
        style={{
          fontFamily: "var(--font-display)",
      ...
      <div className="t-label" style={{ color: "var(--muted)", opacity: 0.5 }}>
        {tab === "xo_monthly"
...
export default function LeaderboardScreen({ onBack }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        await new Promise((r) => setTimeout(r, 750)); // min loading feel
  ...
  return (
    ...
    >
      ...
      {!loading && !error && myRecord && <MyRankBar entry={myRecord} />}
      ...
      >
        ...
        {!loading && !error && entries.length === 0 && <EmptyState tab={tab} />}
        ...
        {!loading && !error && !myInTop5 && myRecord && (
