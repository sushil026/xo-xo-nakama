...
type Profile = {
  ...
  rating: number; winStreak: number; bestStreak: number; username: string;
};

const cardBgStyle = (widthPct: string, right: number, bottom: number): React.CSSProperties => ({
  position: "absolute", right, bottom, width: widthPct, height: "auto",
  opacity: 0.12, pointerEvents: "none",
  maskImage: "linear-gradient(to right, transparent 0%, black 35%)",
  WebkitMaskImage: "linear-gradient(to right, transparent 0%, black 35%)",
});

function StatCell({ value, label, align = "left" }: { value: number | string; label: string; align?: "left" | "center" | "right" }) {
  return (
...
export default function HomeScreen({ isOffline, onPlay, onLocalGame, onProfile, onLeaderboard }: Props) {
    ...
    <div className="screen" role="main">
      ...
      <section style={{ padding: "24px var(--pad) 0", position: "relative", zIndex: 1 }} className="fade-up-1">
        ...
        <h1 className="t-head-lg" style={{ marginTop: 8 }}>
          {loading ? <span className="blink" style={{ color: "var(--muted)" }}>▪▪▪</span> : username}
        </h1>
        <div className="flex items-center gap-8" style={{ marginTop: 10 }}>
          {isOffline ? (
            <span className="t-label" style={{ color: "var(--muted)" }}>Stats unavailable offline</span>
          ) : (
            <>
      ...
      {!isOffline && (
        <div style={{ padding: "16px var(--pad) 0", position: "relative", zIndex: 1 }} className="fade-up-2">
          ...
          <div className="prog-bar">
            <div className="prog-fill" style={{ width: loading ? "0%" : `${winRate}%`, transition: "width 0.6s steps(10)" }} />
          </div>
        </div>
      )}
      ...
      <div style={{ padding: "10px var(--pad) 0", position: "relative", zIndex: 1 }} className="fade-up-4">
        {isOffline ? (
          ...
          >
            ...
            <img src="public/spartan.svg" aria-hidden style={cardBgStyle("78%", -10, -14)} />
            <span className="t-cardnum" style={{ position: "relative", zIndex: 1 }}>⚔</span>
            <div className="t-head" style={{ marginTop: 4, fontSize: 26, position: "relative", zIndex: 1 }}>Local Duel</div>
            <p className="t-body" style={{ marginTop: 4, position: "relative", zIndex: 1 }}>Pass &amp; Play · Same screen</p>
          </button>
        ) : (
          <button
            className="mode-card"
          ...
          >
            ...
            <img src="public/world-map.svg" aria-hidden style={cardBgStyle("78%", -10, -14)} />
            <div className="t-head" style={{ marginTop: 4, fontSize: 26, position: "relative", zIndex: 1 }}>Start</div>
            <p className="t-body" style={{ marginTop: 4, position: "relative", zIndex: 1 }}>Pass &amp; Play · Worldwide · Share Code</p>
          </button>
        )}
      </div>

      {/* Profile + Leaderboard — dimmed offline */}
      <section style={{ padding: "8px var(--pad) 0", display: "flex", gap: 8, position: "relative", zIndex: 1 }} className="fade-up-5">
        <button
          className="mode-card"
          onClick={isOffline ? undefined : onProfile}
          type="button"
          disabled={isOffline}
          style={{ flex: 1, textAlign: "left", padding: "18px 16px", minHeight: 120, overflow: "hidden", opacity: isOffline ? 0.38 : 1, cursor: isOffline ? "not-allowed" : "pointer" }}
        >
          <div className="mode-card-bar" style={{ background: "linear-gradient(to bottom, var(--amber), var(--amber-dim))" }} />
          <span className="mode-card-coord" style={{ fontSize: 9, top: 10, right: 10 }}>02</span>
          <img src="public/fingerprint.svg" aria-hidden style={cardBgStyle("70%", -4, -8)} />
          <div className="t-head" style={{ fontSize: 20, marginTop: 28, position: "relative", zIndex: 1 }}>Profile</div>
          <p className="t-body" style={{ marginTop: 3, fontSize: 11, position: "relative", zIndex: 1 }}>Stats &amp; history</p>
        </button>
        ...
        >
          ...
          <img src="public/crown.svg" aria-hidden style={cardBgStyle("65%", -8, -8)} />
          <div className="t-head" style={{ fontSize: 20, marginTop: 28, position: "relative", zIndex: 1 }}>Ranks</div>
          <p className="t-body" style={{ marginTop: 3, fontSize: 11, position: "relative", zIndex: 1 }}>Global leaderboard</p>
        </button>
      </section>
